...
SELECT
    gen_random_uuid(),   -- Generate a new unique UUID for the root_network row
...
SELECT
    gen_random_uuid(),                -- Generate a new unique UUID for each root_network_node_info row
...
-- Update existing modification_reports to set root_network_node_info_entity_id
UPDATE modification_reports mr
SET root_network_node_info_entity_id = tpn.id
...
AND mr.root_network_node_info_entity_id IS NULL;  -- Only update if the column is NULL


-- Update existing computation_reports to set root_network_node_info_entity_id
...
AND cr.root_network_node_info_entity_id IS NULL;  -- Only update if the column is NULL
