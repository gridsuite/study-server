...
- **Network modification tree:** create, move (cut/paste), duplicate, stash, and restore nodes and subtrees. Two node types: `CONSTRUCTION` and `SECURITY`.
- **Node building:** apply accumulated modifications on a node to produce a concrete network variant. Build statuses (`BUILT`, `NOT_BUILT`, `BUILDING`, `BUILT_WITH_ERROR`) are tracked per (node, root network) pair.
- **12 computation types orchestrated:** Load Flow, Security Analysis, Sensitivity Analysis, Short Circuit Analysis (all-buses and one-bus), Voltage Initialization, Dynamic Simulation, Dynamic Security Analysis, Dynamic Margin Calculation, State Estimation, PCC Min.
...
| `rootNetworkNodeInfo` | Cross-product (node × root network). Stores `variantId`, build statuses (`localBuildStatus`, `globalBuildStatus`), all 11 computation result UUIDs, computation and modification report UUID maps, and excluded modification UUIDs per root network. |
...
**Node Blocking:**
A `blockedNode` flag on `RootNetworkNodeInfoEntity` prevents concurrent destructive operations on a node. Nodes are locked before modification operations and released in a `try/finally` pattern to ensure consistent intermediate states.
